...
A full-stack exchange simulator and algorithmic trading terminal. No external data feeds, no CSV files — the entire market generates itself from scratch. Session-isolated human portfolios, two autonomous trading bots, and a Hyperliquid-inspired trading desk built for live demonstration.
...
- Built-in `dark`/`light` theme toggle with persisted preference (`nb_theme`) and theme-aware chart palettes
...
**WebSocket:** `ws://localhost:8080/ws?session=<session-uuid>`

Each browser tab/session uses a UUID (`trading_session_id` in localStorage). The same UUID is used for:

- WebSocket query param `?session=<uuid>`
- REST header `X-Session-ID: <uuid>` for order endpoints
...
| `portfolio`    | Every 1s + after fills — your session portfolio + both bot portfolios          |
...
| -------- | -------------------- | ----------------------------------------------------- |
| `POST`   | `/orders`            | Place limit or market order (`X-Session-ID` required) |
| `DELETE` | `/orders/{id}`       | Cancel open order (`X-Session-ID` required)           |
...
Required header for `POST /orders` and `DELETE /orders/{id}`:
